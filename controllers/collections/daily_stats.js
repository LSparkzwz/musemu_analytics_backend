...
let query;
let initializedDailyStats = [{
    day: new Date('2020-04-29'),
...
module.exports = {
   /* initializeDailyStats: function(){
           dbAPI.insertDocument(initializedDailyStats,'daily_stats');
    },*/
    ...
    getDailyStats: function (day) {
        return new Promise(async function (resolve, reject) {
            ...
            await dbAPI.getDocument(query, collection).then((dailyStats) => {
                if(dailyStats[0] !== undefined) {
                    resolve(dailyStats[0]);
                }else{
                    resolve(initializedDailyStats[0]);
                }
            });
        });
    },
